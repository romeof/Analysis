...
//   Muon ID
/////
bool is_tth_mu(const pat::Muon& mu, const reco::Vertex& vtx){
...
//   Ele ID
/////
bool is_tth_ele(const pat::Electron& ele, const reco::Vertex& vtx){
 return( ele.electronID("eidLoose")>0.5 && ele.pt()>7 && fabs(ele.eta())<2.5 && ele.gsfTrack().isNonnull() &&
  fabs(ele.gsfTrack()->dxy(vtx.position()))<0.05 && fabs(ele.gsfTrack()->dz(vtx.position()))<0.2 && ele.gsfTrack()->hitPattern().numberOfHits(reco::HitPattern::TRACK_HITS)<=1 &&
  rel_iso_dbc(ele)<0.4 );
}
/////
//   Jet id
